...
extension CoreDataManager {
    ...
    func getAllUniqueTags() -> [String] {
        let notes = fetchNotes() // Uses your existing fetch
        var allTags = Set<String>() // Set ensures no duplicates
        
        for note in notes {
            guard let tagString = note.tags, !tagString.isEmpty else { continue }
            // Split by spaces or commas, remove hashtags, clean up whitespace
            let tags = tagString.components(separatedBy: CharacterSet(charactersIn: " ,"))
            for tag in tags {
